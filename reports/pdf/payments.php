            ...
            <html>
                ...
                <body style="margin:1px;">
                    <div class="list_header" align="center">
                        ...
                        <h5>Reported Incidents Fine Payment List - Generated On ' . date('d M Y') . '</h5>
                    </div>
                    <table border="1" cellspacing="0" width="98%" style="font-size:9pt">
                        <thead>
                            <tr>
                                <th style="width:20%">S/N</th>
                                <th style="width:50%">Payment Ref</th>
                                <th style="width:30%">Payment Amount</th>
                                <th style="width:30%">Payment Method</th>
                                <th style="width:80%">Payment Date Posted</th>
                            </tr>
                        </thead>
                        <tbody>
                            ';
                                ...
                                if (mysqli_num_rows($pdf_sql) > 0) {
                                    while ($row = mysqli_fetch_array($pdf_sql)) {
                                        $html .=
                                            ...
                                            <tr>
                                                <td>' . $cnt . '</td>
                                                <td>' . $row['payment_trans_code'] . '</td>
                                                <td>Ksh ' . number_format($row['payment_amount'], 2) . '</td>
                                                <td>' . $row['payment_method'] . '</td>
                                                <td>' . date('d M Y, g:ia', strtotime($row['payment_date'])) . '</td>
                                            </tr>
                                        ';
                                        $cnt = $cnt + 1;
                                    }
...
$dompdf->render();
$dompdf->stream('Reported Incidents Fine Payments Generated On' . date('d M Y'), array("Attachment" => 1));
